...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
        ...
        function visualizeBases(this,dim_patch)
           
            if length(dim_patch)==1
                % draw 1-D function
                for h=1:1 % two bases
                    figure
                    for p=1:this.n_subspace
                        subplot(this.topo_subspace(1),this.topo_subspace(2),p);
                        plot(this.bases{h}(:,p));
                        set(gca,'xtick',[])
                        set(gca,'ytick',[])
                    end
                ...
                % draw 2-D function like spectrogram
                 for h=1:1 % two bases
                     figure
                    for p=1:this.n_subspace
                        subplot(this.topo_subspace(1),this.topo_subspace(2),p);
                        basis=this.bases{h}(:,p);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %       Reconstruction by specified node
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function X_est=reconstruction(this,X,node)
            coef1 = this.bases{1}(:,node)'*X; %
            %coef2 = this.bases{2}(:,node)'*X;
